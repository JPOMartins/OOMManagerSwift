...
struct ComboBoxEquipments: View {
    let allItems: [EquipmentModel]
    @Binding var selectedItem: EquipmentModel?
    @State private var query: String = ""
    @State private var showSuggestions: Bool = false

    var filteredItems: [EquipmentModel] {
        if query.isEmpty {
            return allItems
        } else {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ZStack(alignment: .trailing) {
                TextField("Select equipment...", text: $query, onEditingChanged: { isEditing in
